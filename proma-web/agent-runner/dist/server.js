...
app.post('/internal/sessions/:id/connect', async (req, res, next) => {
    try {
        res.json(await sessionManager.connectSession(req.params.id, (req.body ?? {})));
    }
    catch (error) {
        next(error);
    }
...
async function writeRunnerStreamEvent(res, event) {
    await writeSSEEvent(res, 'message', event);
}
async function writeSSEEvent(res, event, payload) {
