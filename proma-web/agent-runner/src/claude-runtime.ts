...
type AgentQueryOptions = {
  ...
  baseUrl?: string
  sessionId?: string
  onStreamEvent?: (event: ClaudeRuntimeStreamEvent) => Promise<void> | void
}

class AgentSdkSessionHandle implements ClaudeRuntimeSessionHandle {
  sdkSessionId?: string
  ...
  async connect(_input: ConnectRunnerSessionInput): Promise<{ connectedAt: string }> {
  ...
  async sendMessage(input: RunnerMessageInput): Promise<{ content: string; updatedAt: string }> {
    const result = await runAgentQuery({
  ...
  async streamMessage(input: RunnerMessageInput): Promise<{ chunks: RunnerStreamChunk[]; updatedAt: string }> {
    const result = await runAgentQuery({
      prompt: input.content,
      workspaceId: this.workspaceId,
      modelId: this.modelId,
  ...
  async *streamMessageEvents(input: RunnerMessageInput): AsyncGenerator<ClaudeRuntimeStreamEvent> {
...
async function runAgentQuery(input: AgentQueryOptions): Promise<AgentQueryResult> {
  ...
  let assistantText = ''
  const chunks: RunnerStreamChunk[] = []
  try {
    for await (const message of stream) {
      ...
      if (message && typeof message === 'object' && 'type' in message && message.type === 'assistant' && 'message' in message) {
        ...
        if (text) {
          assistantText += text
          const chunk: RunnerStreamChunk = { content: text, status: 'partial' }
          chunks.push(chunk)
      ...
      if (message && typeof message === 'object' && 'type' in message && message.type === 'result' && 'result' in message && typeof message.result === 'string') {
        ...
        if (message.result) {
          const chunk: RunnerStreamChunk = { content: message.result, status: 'partial' }
          chunks.push(chunk)
          await input.onStreamEvent?.({ chunk })
        }
      }
  ...
  const normalizedChunks = chunks.length === 0
    ? [{ content: normalizedText, status: 'done' as const }]
    : chunks.map((chunk, index) => ({
        ...chunk,
        status: index === chunks.length - 1 ? 'done' as const : 'partial' as const,
      }))
...
export class ClaudeRuntimeAdapter {
  private readonly handles = new Map<string, ClaudeRuntimeSessionHandle>()

  async startSession(input: CreateRunnerSessionInput): Promise<RunnerSessionRecord> {
    const sessionId = randomUUID()
    const now = new Date().toISOString()
    const handle = new AgentSdkSessionHandle(
      sessionId,
      input.workspaceId,
      input.channelId,
      input.modelId,
      input.runtimeConfig.apiKey,
  ...
  async connectSession(session: RunnerSessionRecord, input: ConnectRunnerSessionInput): Promise<RunnerSessionConnectionResult> {
  ...
  async sendMessage(session: RunnerSessionRecord, input: RunnerMessageInput): Promise<RunnerMessageResult> {
    const handle = this.mustGetHandle(session.id)
  ...
  async streamMessage(session: RunnerSessionRecord, input: RunnerMessageInput): Promise<RunnerMessageStreamResult> {
    const handle = this.mustGetHandle(session.id)
  ...
  async *streamMessageEvents(session: RunnerSessionRecord, input: RunnerMessageInput): AsyncGenerator<ClaudeRuntimeStreamEvent> {
    const handle = this.mustGetHandle(session.id)
  ...
  async stopSession(session: RunnerSessionRecord): Promise<RunnerSessionRecord> {
    const handle = this.mustGetHandle(session.id)
    await handle.stop()
  ...
  async restartSession(session: RunnerSessionRecord): Promise<RunnerSessionRecord> {
    const handle = this.mustGetHandle(session.id)
  ...
  private mustGetHandle(sessionId: string): ClaudeRuntimeSessionHandle {
