...
impl FeishuWsClient {
    ...
    async fn get_ws_url(&self) -> Result<String> {
        ...
        log::info!("Got WebSocket URL: {}", data.url);
    ...
    async fn handle_message(&self, data: &[u8], write: &Arc<Mutex<WsWrite>>) -> Result<()> {
        ...
        log::debug!(
            "Frame received: method={}, type={:?}, seq_id={}, log_id={}, service={}, payload_len={}, encoding={}, payload_type={}",
            method,
            msg_type,
            frame.seq_id,
            frame.log_id,
        ...
        match method {
            ...
            _ => {
                log::debug!("Unknown frame method: {}", method);
            }
        }
    ...
    async fn handle_control_frame(&self, frame: &Frame, write: &Arc<Mutex<WsWrite>>) -> Result<()> {
        ...
        match msg_type.as_deref() {
            ...
            Some(MSG_TYPE_EVENT) => {
                log::debug!("Received event in control frame");
                self.handle_data_frame(frame, write).await?;
            }
            _ => {
                log::debug!("Unknown control message type: {:?}", msg_type);
            }
        }
    ...
    async fn handle_data_frame(&self, frame: &Frame, write: &Arc<Mutex<WsWrite>>) -> Result<()> {
        ...
        log::debug!(
            "Data frame headers: type={:?}, encoding={}, payload_type={}",
            msg_type,
            payload_encoding,
            payload_type
        );
    ...
    async fn handle_message_receive(&self, event_data: &serde_json::Value) -> Result<()> {
        log::info!("Message receive: {}", serde_json::to_string_pretty(event_data)?);
    ...
    async fn send_ack(&self, frame: &Frame, write: &Arc<Mutex<WsWrite>>) -> Result<()> {
        ...
        if message_id.is_none() || sum.is_none() || seq.is_none() {
            log::debug!("Missing ack headers: message_id={:?}, sum={:?}, seq={:?}", message_id, sum, seq);
            return Ok(());
        }
